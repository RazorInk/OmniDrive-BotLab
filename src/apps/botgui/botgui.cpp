...
void reset_odometry_pressed(GtkWidget* button, gpointer gui);
...
void BotGui::resetOdometry(void)
{
    reset_odometry_t odomPose;
    odomPose.x = slamPose_.x;
    odomPose.y = slamPose_.y;
    odomPose.theta = slamPose_.theta;
    lcmInstance_->publish(RESET_ODOMETRY_CHANNEL, &odomPose);;
}
                         ...
                         Point<float> worldPoint)
{
    ...
    else if((event->button_mask & VX_BUTTON1_MASK) && (event->modifiers & VX_SHIFT_MASK))
    {        
        pose_xyt_t new_origin;
        new_origin.x = worldPoint.x;
        new_origin.y = worldPoint.y;
        new_origin.theta = 0.0f;
        
        std::cout << "Setting start position to " << new_origin.x << ',' << new_origin.y << " in odometry frame\n";
        
        lcmInstance_->publish(SLAM_RECENTER_CHANNEL, &new_origin);
    }
...
//     gtk_container_add(GTK_CONTAINER(window), mainPane);
    ...
    resetOdometryButton_ = gtk_button_new_with_label("Reset Odometry");
    gtk_box_pack_start(GTK_BOX(optionsBox_), resetOdometryButton_, FALSE, TRUE, 0);
    
    g_signal_connect(clearTracesButton_, 
                     "clicked", 
                     G_CALLBACK(reset_odometry_pressed), 
                     static_cast<gpointer>(this));
